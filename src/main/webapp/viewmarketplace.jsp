...
<body class="bg">
	...
	<div>
		<h2 style="left: 43%; top: 18%; position: absolute;">Welcome</h2><br><br><br><br><br><br><br><br><br>
		<center><h2> ${ farmerInFo.farmerFullName }</h2></center>
	</div>
	<div style="left:30%;top: 45%;position: absolute;">
		<table>
			<tr>
			...
			<c:forEach items="${ listofcropsbyfarmerid }" var="crop">
				<tr>
					...
					<td><c:out value="${crop.cropQuantity}" /><%="\t\tNo's" %></td>
					<td><c:out value="${crop.cropBasicPrice}" /><%="\t\tRs" %></td>
					<td><c:out value="${crop.cropId}" /></td>
					<td><form action="view.rsvp" method="post">
							<input type="hidden" name="cropId" value="${crop.cropId}">
							<button style="font-size: 20px;background-color: transparent;" type="submit">View</button>
						</form>
				</tr>
			</c:forEach>
		</table>
