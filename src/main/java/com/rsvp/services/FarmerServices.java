...
public class FarmerServices {
	...
	public Farmer fetchFarmerInfo(int userId) throws kisaanException {
		try {
		Farmer farmer=farmerRepository.fetchFarmerInfo(userId);
		return farmer;
		}catch (Exception e) {
	...
	public Login isEmailUnique(String email) throws kisaanException {
		...
		try{
		Login login = farmerRepository.isEmailUnique(email);
		if(login!=null) {
		return login;
		}
		return null;
